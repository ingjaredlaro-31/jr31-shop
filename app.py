...
if not st.session_state.auth:
    ...
    if nav == "📦 GESTIÓN STOCK" and st.session_state.is_admin:
        ...
        with t2:
            if not st.session_state.inv_db.empty:
                ed = st.selectbox("SELECCIONAR PRODUCTO", st.session_state.inv_db['ARTICULO'])
                ...
                with st.form("edit"):
                    ...
                    if col_b1.form_submit_button("💾 ACTUALIZAR"):
                        ...
                        st.rerun()
                    if col_b2.form_submit_button("🗑️ ELIMINAR ARTÍCULO"):
    ...
    # --- GASTOS (CON FUNCIÓN DE BORRADO) ---
        ...
        if not st.session_state.expenses_db.empty:
            idx_del = st.selectbox("SELECCIONAR GASTO PARA BORRAR (ERROR)", st.session_state.expenses_db.index)
            if st.button("❌ ELIMINAR REGISTRO SELECCIONADO"):
                st.session_state.expenses_db = st.session_state.expenses_db.drop(idx_del).reset_index(drop=True)
                st.rerun()
    ...
    # --- TERMINAL VENTA ---
        ...
        with col_v2:
            if st.session_state.cart:
                st.table(pd.DataFrame(st.session_state.cart)[["ART", "QTY", "SUB"]])
                rem = st.selectbox("Quitar:", range(len(st.session_state.cart)), format_func=lambda x: st.session_state.cart[x]['ART'])
    ...
    # --- DASHBOARD ---
        ...
        st.markdown("<h1 style='font-family:Orbitron; text-align:center;'>PANORAMA</h1>", unsafe_allow_html=True)
        ...
        with c2: st.markdown(f'<div class="exec-card" style="border-top:4px solid #2E8B57;"><p class="metric-title">CARTERA</p><p class="metric-value" style="color:#2E8B57;">${d:,.0f}</p></div>', unsafe_allow_html=True)

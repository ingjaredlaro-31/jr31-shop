...
# --- 2. INICIALIZACIÓN DE DATOS (PROTECCIÓN ANTI-ERRORES) ---
# Se ejecuta al principio de todo para evitar el KeyError
...
if 'clients_db' not in st.session_state:
    st.session_state.clients_db = pd.DataFrame([{"ID": "JR-000", "NOMBRE": "VENTA MOSTRADOR (EFECTIVO)", "DIRECCION": "N/A", "TELEFONO": "N/A", "DEUDA": 0.0}])
...
# --- 3. DISEÑO SUPREME CYBER-EXECUTIVE (CSS) ---
st.markdown("""
    <style>
    ...
    .stApp { 
        ...
        background-image: linear-gradient(rgba(46, 139, 87, 0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(46, 139, 87, 0.05) 1px, transparent 1px);
    ...
    div[role="radiogroup"] label { font-size: 1.8rem !important; font-weight: 900; color: #FFFFFF !important; margin-bottom: 20px !important; }
...
if not st.session_state.auth:
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        menu_items = ["📊 DASHBOARD", "🛒 VENTAS", "👤 CARTERA"]
        if st.session_state.is_admin:
            menu_items += ["📦 STOCK", "💸 GASTOS", "📝 REPORTES"]
        
        nav = st.radio("SELECCIONE:", menu_items)
        if st.button("SALIR"):
    ...
    # --- STOCK (ALTA Y EDICIÓN) ---
        ...
        with t2:
            if not st.session_state.inv_db.empty:
                ...
                with st.form("ed"):
                    nq = st.number_input("STOCK", value=int(st.session_state.inv_db.at[idx, 'STOCK']))
                    np = st.number_input("PVP", value=float(st.session_state.inv_db.at[idx, 'PVP_JR31']))
                    if st.form_submit_button("ACTUALIZAR"):
                        st.session_state.inv_db.at[idx, 'STOCK'] = nq
                        st.session_state.inv_db.at[idx, 'PVP_JR31'] = np
                        st.rerun()
    ...
    # --- GASTOS Y REPORTES ---
        ...
        if st.button("📄 GENERAR PDF"):
            st.download_button("📥 DESCARGAR", get_pdf_report(), "JR31_Master.pdf")
